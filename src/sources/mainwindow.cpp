...
void MainWindow::setTask()
{
	...
	QStringList items;
	items << QString("2D Object Detection");
	items << QString("2D Semantic Segmentation");
	items << QString("3D Object Detection");
...
{
	QString data_dir = QFileDialog::getExistingDirectory(this, "Select Top Driectory of Verification Data", QDir::currentPath());
	// QString data_dir = "/home/kana/Documents/Project/TS/Data/PreTrained_Test/Dataset/3DOD_Pretrained";
...
void MainWindow::clearLayouts()
{
	...
	clearLayout(ui->net1Layout1);
	clearLayout(ui->net1Layout2);
	clearLayout(ui->net2Layout1);
	clearLayout(ui->net2Layout2);
...
void MainWindow::setAchieve(float AP1, float AP2)
{
	...
	if (AP1 <= 0.0)
		...
		achieve1 = float(AP1 / net1_achieve + 1e-9) * 100;
	if (AP2 <= 0.0)
		achieve2 = 0.0;
	else
		achieve2 = float(AP2 / net2_achieve + 1e-9) * 100;
	if (achieve1 > 100)
		achieve1 = 100;
	if (achieve2 > 100)
		achieve2 = 100;
...
void MainWindow::setNetAPs(int type, vector<pair<QString, float>> mAP)
{
	...
	{
		if (task_idx == 1)
			chart->setTitle("Network1's each Class's IOU(%)");
		else
			chart->setTitle("Network1's each Class's AP(%)");
	}
	else if (type == 2)
	{
		if (task_idx == 1)
			chart->setTitle("Network2's each Class's IOU(%)");
		else
			chart->setTitle("Network2's each Class's AP(%)");
	}
...
int MainWindow::getLOD(float _net1, float _net2)
{
	...
	int LOD = 0;
	if (_net1 > th && _net2 > th)
		LOD = 1;
	else if (_net1 > th && _net2 < th)
		LOD = 2;
	else if (_net1 < th && _net2 > th)
		LOD = 2;
	else if (_net1 < th && _net2 < th)
...
{
	if (task_idx == 0)
		od2d->saveAccept(storage_path);
	else if (task_idx == 1)
		ss2d->saveAccept(storage_path);
	else if (task_idx == 2)
		od3d->saveAccept(storage_path);
}
void MainWindow::dataReject()
{
	if (task_idx == 0)
		od2d->saveReject(storage_path);
	else if (task_idx == 1)
		ss2d->saveReject(storage_path);
	else if (task_idx == 2)
		od3d->saveReject(storage_path);
}
